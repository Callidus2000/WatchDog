...
$parentModule = 'WatchDog'
if (-not $ModuleName) { $ModuleName = 'WatchDog.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\WatchDog'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\WatchDog.psd1"
$prereqHash = @{
